...
>
> If the delivery team is operating against a 40h mental model, every scope decision — estimation, ticket acceptance, escalation threshold — is based on a budget that doesn't exist. Is anyone on the delivery team aware the contract is 20h, not 40h?
...
### Comment 3 — `sim-managed-services-report-Feb1toFeb16-2026.md` line 12

**Target line:** `| 86d1p9d97 | [Spike] Bot response latency | in qa | 24.00h | 18.38h | -5.62h |`

**Comment:**

> A single ticket estimated at 24h against a 20h monthly budget. The estimate alone exceeds the entire month's allocation. Who approved scoping this?
>
> At 18.38h actual, this one spike consumed 92% of the monthly budget. The underlying issue — bot response latency — traces to agent conversation design from the build phase. This is a design defect being absorbed by the MS budget. At 20h/month, one design defect burns an entire month.

---

### Comment 4 — `sim-managed-services-report-Feb1toFeb16-2026.md` line 13
...
### Comment 5 — `sim-managed-services-report-Feb1toFeb16-2026.md` line 16
...
### Comment 6 — `sim-managed-services-report-Feb1toFeb16-2026.md` line 18

**Target line:** `| **TOTAL** | | | **55.00h** | **40.90h** | |`

**Comment:**

> 55h of estimates accepted against a 20h monthly budget — 278% of budget at the estimate level before any work began.
>
> This report covers Feb only. January added another 25.50h of estimates (6 tickets). Combined Jan-Feb: **70.50h estimated against 40h total budget (176%).**
>
> At the actuals level with carry-forward:
>
...
>
> The engagement consumed in 7 weeks what the contract allocates for 4 months.

---

### Comment 7 — `sim-managed-services-report-Feb1toFeb16-2026.md` line 55

**Target line:** `| Total Actual | Manazer | 40.90h |`

**Comment:**

> This is the Feb figure only. January actuals were 51.97h (Manazer) — 260% of the monthly budget. The combined picture is 79.49h actual against 40h of total 2-month budget.
>
> January consumed 2.6x the monthly budget. Was the January overrun flagged to anyone — client or management — before February started?
...
> At 204.5% utilization (Feb only — cumulative is 199% of 2-month total):
>
> - Manazer has no budget configured for this project (`budget_minutes: nil`, `budget_cents: nil`)
> - No billing rate configured (`hourly_billing_rate: nil`)
...
>
> No billing rate is configured on the SIM MS project. Without it, we cannot calculate whether the 18.51h Ganesh contributed to SIM MS generated positive or negative margin. If the contract is 20h/month at $140/hr (implied from delivery phase), revenue is $2,800/month. Ganesh's SIM MS cost alone: ~$555. But there's no way to validate this without a billing rate in the system.
